...
import com.jaycong.boot.capability.mail.constant.MailTlsMode;
...
@Schema(description = "管理端更新邮件通道请求")
public record AdminMailChannelUpdateRequest(
        @Schema(description = "通道名称", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "通道名称不能为空")
        @Size(max = 64, message = "通道名称长度不能超过64")
        String channelName,
        @Schema(description = "SMTP主机", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "SMTP主机不能为空")
        @Size(max = 128, message = "SMTP主机长度不能超过128")
        String smtpHost,
        @Schema(description = "SMTP端口", requiredMode = Schema.RequiredMode.REQUIRED)
        ...
        String smtpUsername,
        @Schema(description = "SMTP密码，为空则表示不更新")
        ...
        MailTlsMode tlsMode,
        @Schema(description = "发件人名称", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "发件人名称不能为空")
        @Size(max = 128, message = "发件人名称长度不能超过128")
        String fromName,
        @Schema(description = "发件人邮箱", requiredMode = Schema.RequiredMode.REQUIRED)
